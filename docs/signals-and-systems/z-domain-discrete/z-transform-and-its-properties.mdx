...
For convenience, if we write $C$, we means that a circle of radius one.
...
$$
x[t] \ast y[t] = \sum_{m = -\infty}^{+\infty} x[t - m] y[m]
$$
...
= \sum_{t = 0}^{+\infty} \sum_{m = -\infty}^{+\infty} x[t - m] z^{-(t - m)} y[m] z^{-m} \\
= X(z) Y(z)
$$
...
$$
x[t] \ast y[t] \xrightarrow{\mathcal{Z}} X(z) Y(z) \quad e^{\sigma_0 + \sigma_1} < |z|
$$

For two side transform,

$$
x[t] \ast y[t] \xrightarrow{\pm \mathcal{Z}} X(z) Y(z) \quad e^{\sigma_0 + \sigma_1} < |z| < e^{\beta_0 + \beta_1}
$$
