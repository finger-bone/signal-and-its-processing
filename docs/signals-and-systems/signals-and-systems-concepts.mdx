...
If a signal is a function defined over a continuous domain, because the variable in it is continuous, it is called continuous signal.

Discreet signal is a sequence defined only on certain points (usually at integer values).
