...
Please note that this solution is only dependent on $f^{(n)}(-0)$. And thus there is no need to 
...
The example doesn't have any actual jump. If there was a jump value, it will came out as $u(t)$ after the reverse transformation.
